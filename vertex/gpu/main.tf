...
resource "google_notebooks_instance" "vertex_ai_instance" {
  ...
  project  = var.project_id
  location = var.zone
  source = "github.com/hpc-terraform/bob_modules//vertex/gpu"
...
metadata = {
  ...
  startup-script     = <<-EOT
    ...
    curl -o /usr/local/bin/check-load.sh https://raw.githubusercontent.com/hpc-terraform/bob_modules/main/scripts/check-load.sh
    curl -o /usr/local/bin/gcs_fuse.sh https://raw.githubusercontent.com/hpc-terraform/bob_modules/main/scripts/gcs_fuse.sh
    ...
    curl -o /etc/systemd/system/check-load.service https://raw.githubusercontent.com/hpc-terraform/bob_modules/main/scripts/check-load.service
    curl -o /etc/systemd/system/check-load.timer https://raw.githubusercontent.com/hpc-terraform/bob_modules/main/scripts/check-load.timer
    curl -o /etc/systemd/system/gcs_fuse_mount.service https://raw.githubusercontent.com/hpc-terraform/bob_modules/main/scripts/gcs_fuse_mount.service
